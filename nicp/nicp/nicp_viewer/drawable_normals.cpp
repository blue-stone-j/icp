...
DrawableNormals::DrawableNormals() :
  Drawable()
{
  _parameter      = 0;
  _points         = 0;
...
void DrawableNormals::draw()
{
  ...
  if (_normalDrawList && normalsParameter && normalsParameter->show() && normalsParameter->normalLength() > 0.0f)
  {
    glPushMatrix();
...
void DrawableNormals::updateNormalDrawList()
{
  ...
  if (_normalDrawList && _points && _normals && normalsParameter && normalsParameter->normalLength() > 0.0f)
  {
