...
class Cloud
{
 public:
  ...
  /* the constness of the instance of this class on which you are calling the method determines which version of
     point() is used.
  */
  ...
  void projectRGB(RGBImage &dest, const IndexImage &indices) const; // make colorful image

  void unprojectRGB(const RGBImage &rgb, const IndexImage &indices); // reset color of image to black
...
void voxelize(Cloud *model, float res); // res: resolution
