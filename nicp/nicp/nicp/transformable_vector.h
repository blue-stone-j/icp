...
struct TransformableVector : public std::vector<Transformable, Eigen::aligned_allocator<Transformable>>
{
   ...
   */
  TransformableVector() :
    std::vector<Transformable, Eigen::aligned_allocator<Transformable>>() {}
   ...
   */
  TransformableVector(size_t s) :
    std::vector<Transformable, Eigen::aligned_allocator<Transformable>>(s) {}
  ...
  inline void transform(TransformableVector &dest, const OtherDerived &m) const
  {
    ...
    for (size_t i = 0;
         i < std::vector<Transformable, Eigen::aligned_allocator<Transformable>>::size();
         ++i, ++tSrc, ++tDest)
    {
      *tDest = m * (*tSrc);
    }
