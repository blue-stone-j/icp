...
bool Cloud::load(Eigen::Isometry3f &T, istream &is)
{
  ...
  while (k < _points.size() && is.good())
  {
    ...
    k++;
  } // endwhile: have read from ifstream.
  return is.good();
}
...
{
  cloud.transformInPlace(T); // update infomation according to new isometry
...
void voxelize(Cloud *model, float res)
{
  ...
  {
    voxels[i] = IndexTriplet(model->points()[i], i, ires);
  }
  Cloud sparseModel;
  ...
  for (size_t i = 0; i < voxels.size(); i++)
  {
    ...
    // k<0 means no voxel has been traversed
  ...
  *model = sparseModel;
  ...
  {
    StatsAccumulator &statsAcc = statsAccs[i];
    if (statsAcc.acc > 1) // if more than one points in this voxel, compress these points into one point
    {
    ...
    } // endif: more than one points in this voxel
  } // endfor: have traversed all voxels and added all new points
}
} // namespace nicp
