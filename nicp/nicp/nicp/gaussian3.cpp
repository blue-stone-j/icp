                                      ...
                                      float dmax, float baseline, float alpha)
{
  ...
  for (int c = 0; c < depthImage.cols; c++)
  {
    for (int r = 0; r < depthImage.rows; r++, f++)
    {
      ...
      Point mean;
      pointProjector.unProject(mean, r, c, *f); // get 3D coordinate from 2D coordinate
      float z          = mean.z();
      ...
      Matrix3f cov = J * imageCovariance * J.transpose(); // transform covariance from 2D to 3D coordinate
